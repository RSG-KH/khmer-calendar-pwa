...
When `showGanzhi` is on, the dialog table has columns **year, month, day, hour**. The hour pillar is computed for Today in the selected Today time zone; for other dates, the hour column displays an em dash (`—`). The first column header is **`☯️ 干支`**, above the sign and clash row labels. It is not a separate caption. The first header cell has zero left padding so its 24px symbol slot and label align with the lotus and Western Big 3 rows. The table has `aria-label="干支"` and can scroll horizontally in a narrow dialog.

When `showWesternZodiac` is on, the dialog renders the Western Zodiac Big 3 table replacing the former single text line. The table has columns **Sun, Moon, Rising sign**. The rising sign is computed for Today in the selected Today time zone; for other dates, the rising column displays an em dash (`—`). The first column header is **`☸️ Big 3`**, above the single body row **`Sign`**. `useEmojiForWesternZodiac` toggles between localized sign names (`false`) and standard emoji symbols (`true`). The first header cell has zero left padding so its 24px symbol slot aligns with the lotus and Ganzhi headings.
...
| Ganzhi columns | Year/month/day/hour use the same engine pillars for the sign and opposing clash rows. When not Today, the hour column displays `—`. Header/data cells have a 52px minimum width; the first header column has a 65px minimum. |
| Western Big 3 columns | Sun/Moon/Rising sign computed via `calculateHoroscope`. When not Today, the rising column displays `—`. First header is `☸️ Big 3`; body row is `Sign`. |
