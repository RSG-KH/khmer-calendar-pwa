...
`WesternZodiac.ts` resolves the selected civil date and time in the selected zone before passing that date's UTC offset to `calculateHoroscope`. It uses representative city coordinates for known IANA zones, including Phnom Penh and Brussels. Unknown zones use their offset's central meridian and Phnom Penh's latitude as an approximation. A time zone does not identify the user's exact location, and different zones can still yield the same Rising sign at a particular time. A nonexistent local time during a daylight-saving transition has no Rising sign; Sun and Moon fall back to noon.
